...
import { Send, Trash2, CornerDownRight, Check, Lightbulb, Undo2 } from "lucide-react";
import { createComment, deleteComment, toggleReaction } from "@/lib/actions/profile";
...
export function CommentsSection({
  ...
  const { attachments, uploading, error: uploadError, addFiles, removeAttachment, reset: resetAttachments } = useAttachmentUpload();
  ...
  function handleComment() {
    ...
    startTransition(async () => {
      try {
        await createComment(demandId, content, undefined, attachments);
        setContent("");
        resetAttachments();
        router.refresh();
      } catch (e) {
    ...
    <div className="rounded-xl border border-navy-100 dark:border-navy-800 bg-white dark:bg-card shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-1">
        ...
        <div className="mt-5 flex gap-3 items-end pt-4 border-t border-navy-50 dark:border-navy-800/60">
          ...
          <div className="flex shrink-0 gap-1.5 mb-0.5">
            <AttachmentPicker onSelect={addFiles} disabled={uploading || pending} compact />
            <button
              type="button"
              disabled={pending || uploading || (!content.trim() && attachments.length === 0)}
...
function CommentItem({
  ...
  const [pending, startTransition] = useTransition();
  const router = useRouter();
  const { attachments: replyAttachments, uploading: replyUploading, error: replyUploadError, addFiles: addReplyFiles, removeAttachment: removeReplyAttachment, reset: resetReplyAttachments } = useAttachmentUpload();
  ...
  function handleReply() {
    ...
    startTransition(async () => {
      await createComment(demandId, replyContent, comment.id, replyAttachments);
      setReplyContent("");
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className={cn(
          ...
          {comment.attachments.length > 0 && (
            <div className="mt-2">
              <AttachmentGallery attachments={comment.attachments} canDelete={isOwn || isDevTeam} />
            </div>
          )}
        </div>
        ...
        {replying && (
          <div className="mt-2 space-y-1.5">
            <div className="flex gap-2">
              ...
              />
              <AttachmentPicker onSelect={addReplyFiles} disabled={replyUploading || pending} compact />
              <button
                type="button"
                disabled={pending || replyUploading || (!replyContent.trim() && replyAttachments.length === 0)}
                onClick={handleReply}
            ...
            <AttachmentChips attachments={replyAttachments} onRemove={removeReplyAttachment} uploading={replyUploading} />
            {replyUploadError && <p className="text-xs text-red-500">{replyUploadError}</p>}
          </div>
        )}
